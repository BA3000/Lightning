...
public class ConnectViaRoomId : MonoBehaviour
{
    ...
    private async void Start()
    {
        ...
        //获取真实房间号
        var realRoomId = await BApi.GetRoomIdInfo(roomId);
        roomId = realRoomId;

        //获取房间信息
        var websocketInfo = await BApi.GetWebsocketInfoViaPass(roomId);
        var json = JsonConvert.SerializeObject(websocketInfo);
        //打印到控制台日志
        Debug.Log(json);
        m_WebSocketBLiveClient = new WebSocketBLiveClient(websocketInfo);
